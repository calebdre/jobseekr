...
interface HackerNewsProcessedCommentProps {
...
export default function HackerNewsProcessedComment({ comment, userId, resumeText, preferences }: HackerNewsProcessedCommentProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  const [isAnalyzing, setIsAnalyzing] = useState(false);

  // Get existing analysis for this user/comment
  const existingAnalysis = useQuery(api.hackernews.getUserAnalysis, {
    userId,
    commentId: comment._id as any, // Use the Convex _id
  });
  ...
  const canAnalyze = hasResume && hasPreferences;
